...
class Person extends MdbBase
{
    ...
    protected function filmograf(&$res, $type)
    {
        ...
        if (!empty($match) && preg_match_all('!<div class="filmo-row.*?>\s*(.*?)\s*</div!ims', $match[1], $matches)) {
            ...
            for ($i=0;$i<$mc;++$i) {
                ...
                $res[] = ["mid"=>$mov[1],"name"=>$mov[2],"year"=>$year,"title_type" => $type,"chid"=>$chid,"chname"=>trim($chname),"addons"=>$addons];
            }
        }
    }
    ...
    public function born()
    {
        if (empty($this->birthday)) {
            if (preg_match('|Born</td>(.*)</td|iUms', $this->getPage("Bio"), $match)) {
                ...
                $this->birthday = ["day" => @$daymon[2], "month" => @$daymon[3], "mon" => @$daymon[1], "year" => @$dyear[1], "place" => @$dloc[1]];
            }
        }
    ...
    public function died()
    {
        if (empty($this->deathday)) {
            ...
            if (preg_match('|Died</td>(.*?)</td|ims', $this->page["Bio"], $match)) {
                ...
                $this->deathday = ["day"=>@$daymon[2],"month"=>@$daymon[3],"mon"=>@$daymon[1],"year"=>@$dyear[1],"place"=>@trim(strip_tags($dloc[1])),"cause"=>@$dcause[1]];
            }
        }
             ...
             if (preg_match('!(\(<a href="/date/(?<month>\d+)-(?<day>\d+).*>\s*\d+\s*(?<monthname>.*)<.*)?\s*(&nbsp;)?\s*(?<year>\d{4})\s+-!ms', $matches[0][0], $match)) { // from date
               $from = ["day"=>$match['day'],"month"=>$match['month'],"mon"=>$match['monthname'],"year"=>$match['year']];
                 ...
                 $from = ["day"=>'',"month"=>'',"mon"=>'',"year"=>''];
                    ...
                    $this->spouses[] = ['imdb'=>$mid,'name'=>$nam,'from'=>$from,'to'=>$to,'comment'=>$comment,'children'=>$children];
                }
            }
            if (!$found) {
                return $this->spouses;
        ...
        if (empty($this->bio_salary)) {
            ...
            if (preg_match_all("/<tr.*?<td.*?>(.*?)<\/td>.*?<td.*?>(.*?)<\/td>/ms", $block, $matches)) { // for each table row
                ...
                for ($i=0;$i<$mc;++$i) {
                    ...
                    $this->bio_salary[] = ["movie"=>$movie,"salary"=>$matches[2][$i]];
                }
            }
        }
        return $this->bio_salary;
        ...
        if (empty($this->pub_prints)) {
            ...
            for ($i = 1; $i < $pc; ++$i) {
                if (preg_match('/(.*).\s*<i>(.*)<\/i>\s*((.*):|)((.*),|)\s*((\d+)\.|)\s*ISBN\s*<a href="(.*)">(.*)<\/a>/iU', $arr[$i], $match)) {
                    $this->pub_prints[] = ["author" => $match[1], "title" => htmlspecialchars_decode($match[2]), "place" => trim($match[4]), "publisher" => htmlspecialchars_decode(trim($match[6])), "year" => $match[8], "isbn" => $match[10], "url" => $match[9]];
                } elseif (preg_match('/(.*).\s*<i>(.*)<\/i>\s*((.*):|)((.*),|)\s*((\d+)\.)/iU', $arr[$i], $match)) {
                    $this->pub_prints[] = ["author" => $match[1], "title" => htmlspecialchars_decode($match[2]), "place" => trim($match[4]), "publisher" => htmlspecialchars_decode(trim($match[6])), "year" => $match[8], "isbn" => "", "url" => ""];
                }
            }
        }
    ...
    protected function parsepubmovies(&$res, $header)
    {
        ...
        for ($i = 0; $i < $pc; ++$i) {
            if (preg_match('/href="\/title\/tt(\d+)\/">(.*)<\/a>\s*(\((\d+)\)|)/', $arr[$i], $match)) {
                $res[] = ["imdb" => $match[1], "name" => $match[2], "year" => $match[4]];
            }
        }
    }
    ...
    protected function parsearticles($title)
    {
        ...
        foreach ($matches[0] as $row) {
            if (@preg_match('|<td.*?>(.*?)</td>.*<td.*?>(.*?)</td>|ms', $row, $match)) {
                ...
                $datum = ["day" => $dat[1], "month" => trim($dat[2]), "mon" => $this->monthNo(trim($dat[2])), "year" => trim($dat[3]), "full" => trim($dat[0])];
                if (strlen($dat[0])) {
                ...
                $res[] = ["inturl" => '', "name" => trim(strip_tags($match[1])), "date" => $datum, "details" => trim($match[2]), "auturl" => '', "author" => $resauthor];
            }
        }
        return $res;
    }
    ...
    {
        $this->SearchDetails = ["role"=>$role,"mid"=>$mid,"moviename"=>$name,"year"=>$year];
    }
