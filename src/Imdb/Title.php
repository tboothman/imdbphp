...
class Title extends MdbBase
{
    ...
    public function yearspan()
    {
        if (empty($this->main_yearspan)) {
            $this->main_yearspan = ['start'=>$this->year(),'end'=>$this->endyear()];
        }
        ...
        if (empty($this->movieruntimes)) {
            ...
            foreach (explode('|', strip_tags($rt)) as $runtimestring) {
                if (preg_match("/(\d+) min/", $runtimestring, $matches)) {
                    ...
                    $this->movieruntimes[] = ["time"=>$runtime, "country"=>'', "comment"=>'', "annotations" => $annotations];
                }
            }
        }
    ...
    public function comment_split()
    {
        if (empty($this->split_comment)) {
            ...
            if (@preg_match('!<strong[^>]*>(.*?)</strong>.*?<div class="comment-meta">\s*(.*?)\s*\|\s*by\s*(.*?</a>).*?<p[^>]*>(.*?)\s*</div!ims', $this->main_comment, $match)) {
                ...
                $this->split_comment = ["title"=>$match[1],"date"=>$match[2],"author"=>["url"=>$author[1],"name"=>$author[2]],"comment"=>trim($match[4])];
                ...
                $this->split_comment = ['title'=>'','date'=>$match[1],'author'=>["url"=>$author[1],"name"=>$author[2]],"comment"=>trim($match[3])];
            }
        }
    ...
    public function languages()
    {
        if (empty($this->langs)) {
            if (preg_match_all('!href="/search/title\?.+?primary_language=([^&]*)[^>]*>\s*(.*?)\s*</a>(\s+\((.*?)\)|)!m', $this->getPage("Title"), $matches)) {
                ...
                $mc = count($matches[2]);
                for ($i = 0; $i < $mc; $i++) {
                    $this->langs_full[] = ['name' => $matches[2][$i], 'code' => $matches[1][$i], 'comment' => trim($matches[4][$i])];
                }
            }
        }
        return $this->langs;
        ...
        if (empty($this->main_creator)) {
            ...
            if (@preg_match("#Creators?:\</h4\>[\s\n]*(.*?)(</div|<a class=\"tn15more)#ms", $this->page["Title"], $match)) {
                if (preg_match_all('#/name/nm(\d+).*?><span.+?>(.*?)</span#s', $match[1], $matches)) {
                    for ($i = 0; $i < count($matches[0]); ++$i) {
                        $this->main_creator[] = ['name' => $matches[2][$i], 'imdb' => $matches[1][$i]];
                    }
                }
            }
        }
    ...
    public function prodNotes()
    {
        if (empty($this->main_prodnotes)) {
            ...
            if (preg_match('!<b>Updated:\s*</b>\s*(\d+)\s*(\D+)\s+(\d{4})!ims', $match[1], $tmp)) {
                $update = ["day"=>$tmp[1],"month"=>$tmp[2],"mon"=>$this->monthNo($tmp[2]),"year"=>$tmp[3]];
            } else {
                $update = [];
            }
            ...
            $this->main_prodnotes = ["status"=>$status,"statnote"=>$statnote,"lastUpdate"=>$update,"more"=>$more,"note"=>$note];
        }
        return $this->main_prodnotes;
    }
        ...
        if (empty($this->split_plot)) {
            ...
            foreach ($this->plot_plot as $plot) {
                if (preg_match('!(?<plot>.*?)\n-\n<a href="(?<author_url>.*?)">(?<author_name>.*?)<\/a>!ims', $plot, $match)) {
                    $this->split_plot[] = ["plot"=>$match['plot'],"author"=>["name"=>$match['author_name'],"url"=>$match['author_url']]];
                } else {
                    $this->split_plot[] = ["plot"=>$plot,"author"=>["name"=>'',"url"=>'']];
                }
            }
        }
        ...
        if (empty($this->goofs)) {
            ...
            if (@preg_match_all('@<h4 class="li_group">(.+?)(!?&nbsp;)</h4>\s*(.+?)\s*(?=<h4 class="li_group">|<div id="top_rhs_wrapper")@ims', $this->page["Goofs"], $matches)) {
                ...
                for ($i=0;$i<$gc;++$i) {
                    ...
                    $ic = count($goofy[0]);
                    for ($k=0;$k<$ic;++$k) {
                        $this->goofs[] = ["type"=>$matches[1][$i],"content"=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', trim($goofy[2][$k]))];
                    }
                }
            }
        }
                        ...
                        if (!empty($matches[1])) {
                            foreach ($matches[1] as $quote) {
                                if (@preg_match('!href="([^"]*)"\s*>.+?character">(.*?)</span.+?:(.*)!', $quote, $match)) {
                                    $this->split_moviequotes[$i][] = ['quote'=>trim(strip_tags($match[3])),'character'=>['url'=>$match[1],'name'=>$match[2]]];
                                } else {
                                    $this->split_moviequotes[$i][] = ['quote'=>trim(strip_tags($quote)),'character'=>['url'=>'','name'=>'']];
                                }
                            }
                        }
                    }
        ...
        if (empty($this->trailers)) {
            ...
            if ($has_trailers !== false) {
                ...
                foreach ($doc->getElementsByTagName('li') as $trailerNode) {
                    ...
                    if ($full) {
                        $this->trailers[] = ['title' => $title, 'url' => $url, 'resolution' => $res, 'lang' => '', 'restful_url' => ''];
                    } else {
                        $this->trailers[] = $url;
                    }
                }
            ...
            if (preg_match_all('!<li>(.+?)</li>!ims', $match[1], $matches)) {
                ...
                for ($i=0;$i<$mc;++$i) {
                    if (preg_match('!<a .*href="(?<url>.+?)".*?>(?<site>.*?) - (?<desc>.*) \((?<type>.*?)\)</a>!s', $matches[1][$i], $entry)) {
                        ...
                        $res[] = ['site'=>$entry['site'], 'url'=>$entry['url'], 'type'=>$entry['type'], 'desc'=>$entry['desc']];
                    } elseif (preg_match('!<a .*href="(?<url>.+?)".*?>(?<site>.*?) - (?<desc>.+)</a>!s', $matches[1][$i], $entry)) {
                        $entry['url'] = $this->convertIMDBtoRealURL($entry['url']);
                        $res[] = ['site'=>$entry['site'], 'url'=>$entry['url'], 'type'=>'', 'desc'=>$entry['desc']];
                    } elseif (preg_match('!<a .*href="(?<url>.+?)".*?>(?<desc>.+)</a>!s', $matches[1][$i], $entry)) {
                        $entry['url'] = $this->convertIMDBtoRealURL($entry['url']);
                        $res[] = ['site'=>'', 'url'=>$entry['url'], 'type'=>'', 'desc'=>$entry['desc']];
                    }
                }
                return $res;
            }
                ...
                for ($i=0;$i<$mc;++$i) {
                    ...
                    if (preg_match_all('|^\s*(.*?)\s+by\s+(<a href[^>]+>.+?</a>)|i', $matches['desc'][$i], $match1)) {
                        for ($k=0;$k<count($match1[0]);++$k) {
                            switch ($match1[1][$k]) {
                                case "Arranged": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'arrangement']; break;
                                case "Composed": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'composer']; break;
                                case "Performed": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'performer']; break;
                                case "Written": $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'writer']; break;
                                case "Written and Produced": {
                                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'writer'];
                                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'producer'];
                                } break;
                                case "Written and Performed": {
                                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'writer'];
                                  $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'performer'];
                                } break;
                                default: $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $match1[2][$k]), 'desc'=>'**'.$match1[1][$k].'**'];
                            }
                        }
                        ...
                        for ($k=0;$k<count($match1[0]);++$k) {
                            ...
                            switch ($match1[1][$k]) {
                                case "Arranged": foreach ($creds as $cred) {
                                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $cred), 'desc'=>'arrangement'];
                                } break;
                                case "Composed": foreach ($creds as $cred) {
                                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $cred), 'desc'=>'composer'];
                                } break;
                                case "Performed": foreach ($creds as $cred) {
                                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $cred), 'desc'=>'performer'];
                                } break;
                                case "Written": foreach ($creds as $cred) {
                                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $cred), 'desc'=>'writer'];
                                } break;
                                case "Written and Produced": foreach ($creds as $cred) {
                                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $cred), 'desc'=>'writer'];
                                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $cred), 'desc'=>'producer'];
                                }
                                   break;
                                case "Written and Performed": foreach ($creds as $cred) {
                                   ...
                                   break;
                                default: foreach ($creds as $cred) {
                                    $s['credits'][] = ['credit_to'=>str_replace('href="/', 'href="https://'.$this->imdbsite.'/', $cred), 'desc'=>'**'.$match1[1][$k].'**'];
                                } break;
                            }
                        }
                    }
                    if (preg_match('|Courtesy of\s+([^<]+)<|i', $matches['desc'][$i], $match)) {
                        $s['credits'][] = ['credit_to'=>$match[1], 'desc'=>'courtesy'];
                    }
                    if (preg_match('|By Arrangement with\s+([^<]+)<|i', $matches['desc'][$i], $match)) {
                        $s['credits'][] = ['credit_to'=>$match[1], 'desc'=>'arrangement'];
                    }
                    if (preg_match('|Under license from\s+([^<]+)<|i', $matches['desc'][$i], $match)) {
                        $s['credits'][] = ['credit_to'=>$match[1], 'desc'=>'license'];
                    }
                    $this->soundtracks[] = $s;
                }
            }
    ...
    protected function parseConnection($conn)
    {
        ...
        if (preg_match_all('!<a href="(.*?)">(.*?)</a>(?:&nbsp;\((\d{4})\))?(.*?<br\s*/>(.*?)\s*</div>)?!ims', $block, $matches)) {
            ...
            for ($i=0;$i<$mc;++$i) {
                ...
                $arr[] = ["mid"=>$mid, "name"=>$matches[2][$i], "year"=>$matches[3][$i], "comment"=>trim($matches[5][$i])];
            }
        }
        return $arr;
    }
    ...
    public function extReviews()
    {
        if (empty($this->extreviews)) {
            ...
            if (preg_match_all('@href="(.*?)"[^>]*>(.*?)</a>@', $block, $matches)) {
                $mc = count($matches[0]);
                for ($i=0;$i<$mc;++$i) {
                    $this->extreviews[$i] = ["url"=>'https://'.$this->imdbsite.$matches[1][$i], "desc"=>$matches[2][$i]];
                }
            }
        }
            ...
            if (! empty($matches[0]) && $mc = count($matches[0])) {
                for ($i=0;$i<$mc;++$i) {
                    ...
                    if (preg_match('!^(\d{1,2})\s(.+?)\s(\d{4})$!s', trim($matches[2][$i]), $match)) { // day, month and year
                        $this->release_info[] = ['country'=>$country,'day'=>$match[1],'month'=>$match[2],'mon'=>$this->monthNo(trim($match[2])),'year'=>$match[3],'comment'=>$comment];
                    } elseif (preg_match('!^(.+?)\s(\d{4})$!s', trim($matches[2][$i]), $match)) { // month and year
                        $this->release_info[] = ['country'=>$country,'day'=>'','month'=>$match[1],'mon'=>$this->monthNo(trim($match[1])),'year'=>$match[2],'comment'=>$comment];
                    } elseif (preg_match('!(\d{4})!', trim($matches[2][$i]), $match)) { // year at least
                        $this->release_info[] = ['country'=>$country,'day'=>'','month'=>'','mon'=>'','year'=>$match[1],'comment'=>$comment];
                    } else {
    ...
    protected function companyParse(&$text, &$target)
    {
        ...
        $mc = count($matches[0]);
        for ($i=0;$i<$mc;++$i) {
            $target[] = ["name"=>$matches[2][$i], "url"=>'https://'.$this->imdbsite.$matches[1][$i], "notes"=>trim($matches[3][$i])];
        }
    }
    ...
    public function officialSites()
    {
        if (empty($this->official_sites)) {
            ...
            foreach ($sites as $site) {
                $this->official_sites[] = ['url'=>$site['url'],'name'=>$site['desc']];
            }
        }
                    ...
                    switch ($col['class'][$k]) {
                        ...
                        case "award_description":
                          ...
                          if (preg_match_all('|<a href\="/name/nm(\d+)[^"]*"\s*>(.*?)</a>|s', $desc, $data)) {
                              $people = isset($data[0][0]) ? array_combine($data[1], $data[2]) : [];
